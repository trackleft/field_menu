    ...
    foreach ($items as $delta => $item) {
      ...
      if ($level > 1) {
        if (count($menu_parameters->activeTrail) >= $level) {
          ...
          $menu_parameters->setRoot($menu_root);
      ...
      // If we're currently following an active menu item, or for menu fields
      ...
      if ($fixed_parent_menu_link_id) {
        ...
        // active menu trail.
        $fixed_parameters = clone $menu_parameters;
        ...
        if ($render_parent) {
          ...
          if (!in_array($fixed_parent_menu_link_id, $menu_parameters->activeTrail) && (int) $level <> 1) {
            return [];
          }
        }
      ...
      if (!empty($tree_render_array['#theme'])) {
        // Add the configuration for use in field_menu_theme_suggestions_menu().
        ...
        $tree_render_array['#field_menu_configuration']['label'] = $title;
        // Remove the menu name-based suggestion so we can control its
        // precedence better in field_menu_theme_suggestions_menu().
